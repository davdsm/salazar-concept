      ...
      <section className="container mt-16 pt-10 relative max-w-none">
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 items-center">
          {clients.map(({ id }, index) => (
            ...
            >
              <div style={{backgroundColor: "#f1f1f1"}} className="client rounded-full w-32 h-32 mx-auto hover:drop-shadow-2xl">
                <img src={`/clients/${index + 1}.png`} style={{scale: "0.7"}}  className="mx-auto rounded-full w-32 h-32 object-contain" />
              </div>
            </AnimationOnScroll>
          ))}
