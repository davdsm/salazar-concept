/* ---------------------------------------------------------------------------
 * Cloth — Canvas UI (https://canvasui.dev), MIT + Commons Clause.
 * Copiado para este projeto, que a licença permite; o que ela proíbe é
 * revendê-lo ou redistribuí-lo como componente.
 *
 * Fonte: https://canvasui.dev/r/cloth-vanilla.json
 *
 * ALTERADO num único ponto ("imageMode"): o componente alimenta a textura
 * com HTML vivo através de drawElementImage(), que exige a flag
 * HTML-in-Canvas do Chrome e desliga-se sem ela. Quando `elements.image` é
 * passado, essa captura é substituída por um upload direto da imagem para a
 * mesma textura. A simulação, os dois passes de shader e as opções ficam
 * intactos — e assim corre em qualquer browser com WebGL2.
 *
 * As linhas alteradas estão marcadas com  // [imageMode]
 * ------------------------------------------------------------------------ */
...
export interface ClothElements {
  ...
  /**
   * [imageMode] Still image to hang on the cloth instead of live HTML.
   * Pass a canvas/image, or a function returning one (re-read on every
   * upload, so it can follow a resize). When set, the html-in-canvas
   * capture path is bypassed and no Chrome flag is needed.
   */
  image?: CanvasImageSource | (() => CanvasImageSource | null);
}
...
export interface ClothInstance {
  ...
  destroy: () => void;
  /** [imageMode] Re-upload the image. Call after changing or resizing it. */
  refresh: () => void;
}
...
export function createCloth(
  ...
  const { source, content, output, image } = elements;
  const imageMode = Boolean(image); // [imageMode]
  ...
  if (htmlInCanvas && !imageMode) { // [imageMode]
  ...
  capture = () => {
    if (imageMode) {
      // [imageMode] mesma textura, mesmos shaders — só muda a origem dos pixels
      const plate = typeof image === "function" ? image() : image;
      if (!plate) return;
      gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
  ...
  function syncCanvasSize() {
    ...
    if (imageMode) contentMaxX = 1; // [imageMode]
    else contentMaxX = Math.min(
      1,
    ...
    if (probeCtx) {
      ...
      while (el) {
        ...
        if (bg && bg !== "transparent") {
          ...
          // Indexado em vez de desestruturado: o tsconfig do projeto tem
          // target es5, que não itera TypedArrays.
  ...
  function start() {
    if (!htmlInCanvas && !imageMode) return; // [imageMode]
  ...
  return {
    ...
    destroy() {
      ...
      if (htmlInCanvas && !imageMode) paintable.onpaint = null; // [imageMode]
    },
  };
}
